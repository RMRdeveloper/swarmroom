...
- Document production installation and add dynamic npm badges.
...
- Renamed the package to `@rmrdeveloper/sideroom-pi`, its Pi command to
  `/sideroom`, and all package-owned role and skill IDs to `sideroom-*`.
- Renamed Pi session trace and UI resource keys from `swarmroom:*` to
  `sideroom:*`.

Historical entries below retain their original Swarmroom names.
